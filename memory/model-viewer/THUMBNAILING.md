...
- Ao instalar em `/usr` via `sudo`, os instaladores removem o thumbnailer legado em `~/.local` quando resolvem `SUDO_USER` com `getent`; caso contrário, avisam a remoção manual. O GNOME prioriza a entrada local, mas o sandbox não enxerga seu executável.
...
3. Sem malhas renderizáveis, a geração falha imediatamente; caso contrário, após dois frames Screenshot lê o render target e grava o PNG.
